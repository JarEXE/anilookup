...
const AllManga = ({ isDarkMode, selectedOption, sortBy }) => {
  ...
  const columns = [
    ...
    },
    {
      ...
      cell: (row) => (
        ...
        >
          <img src={row.image} alt="manga cover" width={75} height={110} />
          {screenWidth > 500 ? (
  ...
  const showDetails = async (mangaId, mangaTitle) => {
    setIsActive(true);
    let kitsuData;

    await fetch(
      `https://kitsu.io/api/edge/manga?filter[text]=${mangaTitle}&page[limit]=1`
    ).then(async (response) => {
